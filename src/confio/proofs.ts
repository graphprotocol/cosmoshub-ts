...
export class ProofSpec {
  static encode(message: ProofSpec, writer: Writer): void {
    const leaf_spec = message.leaf_spec;
    if (leaf_spec !== null) {
      writer.uint32(10);
      writer.fork();
      LeafOp.encode(leaf_spec, writer);
      writer.ldelim();
    }

    const inner_spec = message.inner_spec;
    if (inner_spec !== null) {
      writer.uint32(18);
      writer.fork();
      InnerSpec.encode(inner_spec, writer);
      writer.ldelim();
    }
...
export class InnerSpec {
  static encode(message: InnerSpec, writer: Writer): void {
    const child_order = message.child_order;
    if (child_order.length !== 0) {
      for (let i = 0; i < child_order.length; ++i) {
        writer.uint32(8);
        writer.int32(child_order[i]);
      }
    }
...
export class CompressedBatchProof {
  static encode(message: CompressedBatchProof, writer: Writer): void {
    ...
    const lookup_inners = message.lookup_inners;
    for (let i = 0; i < lookup_inners.length; ++i) {
      writer.uint32(18);
      writer.fork();
      InnerOp.encode(lookup_inners[i], writer);
      writer.ldelim();
    }
  }
